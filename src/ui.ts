...
<body><main>
  <header><h1>Deft SEO Agent</h1><p>Research a site and topic, plan the article, draft sections in parallel with Deft, then run constrained structural and factual review.</p></header>
  <section class="grid" id="workspace">
    <form class="card" id="run-form">
      ...
      <p id="form-note">API keys are read from the local Node process and never enter this page.</p>
    </form>
    <div class="card"><h2>How the agent works</h2><div class="flow" id="flow">
      <div class="step" data-step="research">Research site</div><div class="step" data-step="plan">Plan article</div><div class="step" data-step="draft">Draft sections in parallel</div><div class="step" data-step="structural">Structural edit</div><div class="step" data-step="review">Line edit &amp; review</div>
...
const form=document.querySelector('#run-form'),button=document.querySelector('#run-button'),log=document.querySelector('#log'),notice=document.querySelector('#notice'),result=document.querySelector('#result'),article=document.querySelector('#article');let markdown='';
