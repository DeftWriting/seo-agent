...
function parsePlan(value: unknown): ArticlePlan {
  ...
  const sections = root.sections.map(record).map((section, index) => {
    ...
    return { id: `S${index + 1}`, heading, summary, outline, facts: facts(section.facts) };
  });
  const plan = {
...
export async function planStep(
  ...
  return { plan: parsePlan(response.value), usage: response.usage };
}
